...
import { API_TZP_PRESS_FORM_SHORT } from '../../../infrastructure/const/urls';

export function ChangeStateTzpPressForm({ route }: INavigate<TMember>): ReactElement {
  const { error, isLoading, updateResponse, resetError } = useHttp<ITzpPressform>();
  ...
  useEffect(() => {
    if (entity && urlTransitions) {
      const url = API_TZP_PRESS_FORM_SHORT + '/' + entity.id + urlTransitions;
      updateResponse({
        url,
        method: 'PUT',
