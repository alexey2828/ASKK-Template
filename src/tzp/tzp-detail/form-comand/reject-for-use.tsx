...
type TTZPDetailRejectForUseDto = {
  comment: string;
  masterUserId: string;
};

export const TZPDetailRejectForUse = ({ route }: INavigate<ITzpDetail, TTZPDetailRejectForUseDto>): ReactElement => {
  const { entity } = route.params;
  const [comment, setComment] = useState('');
  const [validComment, setValidComment] = useState(isStringCommon(comment));
  const [tZPDetailRejectForUseDto, setTZPDetailRejectForUseDto] = useState<TTZPDetailRejectForUseDto | null>(null);
  ...
  useEffect(() => {
    if (validComment.valid && masterUser) {
      setTZPDetailRejectForUseDto({
        comment: comment,
        masterUserId: masterUser?.id,
      });
    } else {
      setTZPDetailRejectForUseDto(null);
    }
  ...
  return (
    <View style={CNstyles.FormContainer}>
      <View style={{ margin: 20 }}>
        ...
        <View style={CNstyles.defaultHeight} />
        <View style={CNstyles.defaultHeight} />
        {tZPDetailRejectForUseDto ? (
          <GoNext
            routeName={ENavigationName.CHANGE_STATE_TZP_DETAIL}
            dtoTransitions={tZPDetailRejectForUseDto}
            endpoint={endpointTransition.REJECT_FOR_USE}
            entity={entity}
