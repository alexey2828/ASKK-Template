...
import { API_TZP_PLATE_SHORT_URL } from '../../../infrastructure/const/urls';

export function ChangeStateTzpDetailSample({ route }: INavigate<TMember>): ReactElement {
  const dto = route.params.dtoTransitions || {};
  const entity = route.params.entity;
  const urlTransitions = route.params.endpoint;
  const { error, isLoading, updateResponse, resetError } = useHttp<ITzpDetailSample>();
  const { goBack } = useNavigate<TMember>();

  useEffect(() => {
    if (entity && urlTransitions) {
      const url = API_TZP_PLATE_SHORT_URL + '/' + entity.id + urlTransitions;
      updateResponse({
        url,
        method: 'PUT',
