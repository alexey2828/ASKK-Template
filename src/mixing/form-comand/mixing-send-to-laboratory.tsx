...
export const MixingSendToLaboratory = ({ route }: INavigate<IMixing, null>): ReactElement => {
  const { entity } = route.params;
  const routeName = routes.CHANGE_STATE_MIXING;
  const endpoint = endpointTransition.SEND_TO_LABORATORY;
    ...
    <View style={CNstyles.FormContainer}>
      <View style={{ margin: 20 }}>
        ...
        <GoNext
          ...
          entity={entity}
          isActive={true}
          title={ETransitionsUK.SEND_TO_LABORATORY + '?'}
        />
        <View style={CNstyles.defaultHeight} />
        <GoBack />
      </View>
