...
type TMaterialsTakeToWorkDto = { masterUserId: string };

export const MaterialsTakeToWork = ({ route }: INavigate<IEnginePressform, TMaterialsTakeToWorkDto>): ReactElement => {
  const { entity } = route.params;

  const [masterUser, setMasterUser] = useState<IUser | null>(null);
  const [materialsTakeToWorkDto, setMaterialsTakeToWorkDto] = useState<TMaterialsTakeToWorkDto | null>(null);
  ...
  useEffect(() => {
    if (masterUser) {
      setMaterialsTakeToWorkDto({
        masterUserId: masterUser?.id,
      });
    } else {
      setMaterialsTakeToWorkDto(null);
    }
  }, [masterUser]);
  ...
  return (
    <View style={CNstyles.FormContainer}>
      <View style={{ margin: 20 }}>
        ...
        <View style={CNstyles.defaultHeight} />
        {materialsTakeToWorkDto ? (
          <GoNext
            routeName={ENavigationName.CHANGE_STATE_MATERIALS}
            dtoTransitions={materialsTakeToWorkDto}
