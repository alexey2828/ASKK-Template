...
import { INozzleBlock } from '../entity/nozzle-block';
...
import { TITLES_NOZZLE_BLOCK } from '../const/titles';
import { TITLES_TZP_PRESSFORM } from 'tzp/tzp-pressform/const/titiles';

interface IDetailNozzleBlock extends IModalDetail {
  entity: INozzleBlock;
}

export const DetailNozzleBlock: React.FC<IDetailNozzleBlock> = ({ entity }) => {
      ...
      <View style={CNstyles.BtnContainerDetails}>
        <View style={{ margin: 10 }}>
          <Text style={styles.itemInfo}>
            {TITLES_NOZZLE_BLOCK.number}: <Text style={styles.itemInfoa1}>{entity?.name}</Text>
          </Text>
          <Text style={styles.itemInfo}>
            {TITLES_NOZZLE_BLOCK.formularNumber}: <Text style={styles.itemInfoa1}>{entity?.formularNumber}</Text>
          </Text>
          <Text style={styles.itemInfo}>
            {TITLES_NOZZLE_BLOCK.paintNumber}: <Text style={styles.itemInfoa1}>{entity?.drawingNumber}</Text>
          </Text>
          <Text style={styles.itemInfo}>
            {TITLES_NOZZLE_BLOCK.date}:{' '}
