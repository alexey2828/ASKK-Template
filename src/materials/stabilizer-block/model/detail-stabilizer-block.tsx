...
import { TITLES_STABILIZER_BLOCK } from '../const/titles';
import { TITLES_TZP_PRESSFORM } from 'tzp/tzp-pressform/const/titiles';

interface IDetailStabilizerBlock extends IModalDetail {
  entity: IStabilizerBlock;
}

export const DetailStabilizerBlock: React.FC<IDetailStabilizerBlock> = ({ entity }) => {
      ...
      <View style={CNstyles.BtnContainerDetails}>
        <View style={{ margin: 10 }}>
          <Text style={styles.itemInfo}>
            {TITLES_STABILIZER_BLOCK.number}: <Text style={styles.itemInfoa1}>{entity?.name}</Text>
          </Text>
          <Text style={styles.itemInfo}>
            {TITLES_STABILIZER_BLOCK.formularNumber}: <Text style={styles.itemInfoa1}>{entity?.formularNumber}</Text>
          </Text>
          <Text style={styles.itemInfo}>
            {TITLES_STABILIZER_BLOCK.paintNumber}: <Text style={styles.itemInfoa1}>{entity?.drawingNumber}</Text>
          </Text>
          <Text style={styles.itemInfo}>
            {TITLES_STABILIZER_BLOCK.date}:{' '}
