...
export function ChangeStateComponentBatchPlace({ route }: INavigate<TMember>): ReactElement {
  const dto = route.params.dtoTransitions || {};
  const entity = route.params.entity;
  const urlTransitions = route.params.endpoint;
  const { error, isLoading, updateResponse, resetError } = useHttp<IComponentBatchPlace>();

  const { goBack } = useNavigate<TMember>();

  useEffect(() => {
    if (entity && urlTransitions) {
      const url = API_COMPONENT_BATCH_PLACE_SHORT_URL + '/' + entity.id + urlTransitions;
      updateResponse({
        url,
        method: 'PUT',
        ...
        <View style={{ alignItems: 'center' }}>
          ...
          >
            <View style={Lstyles.btnBackContainer}>
              <Text style={Lstyles.btnBackTitle}>{MainTitles.BACK}</Text>
            </View>
          </TouchableHighlight>
        </View>
      )}
