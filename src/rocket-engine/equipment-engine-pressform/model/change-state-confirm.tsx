...
import { API_EQUIPMENT_ENGINE_PRESSFORM_SHORT_URL } from 'infrastructure/const/urls';
import React, { ReactElement, useEffect } from 'react';
import { Text, View, TouchableHighlight, Image } from 'react-native';

import { IEquipmentEnginePressform } from '../entity/equipment-engine-pressform';

export function ChangeStateEquipmentEnginePressform({ route }: INavigate<TMember>): ReactElement {
  const dto = route.params.dtoTransitions || {};
  const entity = route.params.entity;
  const urlTransitions = route.params.endpoint;
  const { error, isLoading, updateResponse, resetError } = useHttp<IEquipmentEnginePressform>();
  const { goBack } = useNavigate<TMember>();

  useEffect(() => {
    if (entity && urlTransitions) {
      const url = API_EQUIPMENT_ENGINE_PRESSFORM_SHORT_URL + '/' + entity.id + urlTransitions;
      updateResponse({
        url,
        method: 'PUT',
