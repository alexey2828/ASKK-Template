...
export const PutIntoKcPolymerization = ({ route }: INavigate<IEngineCase, TPutIntoKcPolymerization>): ReactElement => {
  ...
  const [putIntoKcPolymerizationDto, setPutIntoKcPolymerization] = useState<TPutIntoKcPolymerization | null>(null);
  ...
  useEffect(() => {
    if (isValidSettingTime && thermalChamber && apparatchikUser && masterUser) {
      ...
      setPutIntoKcPolymerization(null);
    }
  ...
  return (
    <View style={CNstyles.FormContainer}>
      ...
      <View style={{ margin: 20 }}>
        ...
        <View style={styles.border} />
        <View style={CNstyles.defaultHeight} />
        {putIntoKcPolymerizationDto ? (
          <>
            <GoNext
              routeName={ENavigationName.CHANGE_STATE_ENGINE_CASES}
              dtoTransitions={putIntoKcPolymerizationDto}
              endpoint={endpointTransition.PUT_INTO_KC_POLYMERIZATION}
              entity={entity}
              title={ETransitionsUK.commandPutIntoKcPolymerization + '?'}
            />
          </>
        ) : (
          <LockButton />
