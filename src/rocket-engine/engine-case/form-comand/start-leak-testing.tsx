...
type TStartLeakTestingDto = {
  masterUserId: string;
  apparatchikUserId: string;
};

export const StartLeakTesting = ({ route }: INavigate<IEngineCase, TStartLeakTestingDto>): ReactElement => {
  ...
  const [startLeakTestingDto, setStartLeakTestingDto] = useState<TStartLeakTestingDto | null>(null);
  ...
  useEffect(() => {
    if (masterUser && apparatchikUser) {
      setStartLeakTestingDto({
        masterUserId: masterUser?.id,
        apparatchikUserId: apparatchikUser?.id,
      });
    } else {
      setStartLeakTestingDto(null);
    }
      ...
      <View style={{ margin: 20 }}>
        ...
        {startLeakTestingDto ? (
          <GoNext
            ...
            dtoTransitions={startLeakTestingDto}
            endpoint={endpointTransition.START_LEAK_TESTING}
            entity={entity}
            title={ETransitionsUK.commandStartLeakTesting + '?'}
          />
        ) : (
          <LockButton />
        )}
