...
export const PutIntoDrying = ({ route }: INavigate<IEngineCase, TPutIntoDryingDto>): ReactElement => {
  ...
  const [putIntoDryingDto, setPutIntoDryingDto] = useState<TPutIntoDryingDto | null>(null);
        ...
        {putIntoDryingDto ? (
          <>
            <GoNext
              ...
              dtoTransitions={putIntoDryingDto}
              endpoint={endpointTransition.PUT_INTO_DRYING}
              entity={entity}
              title={ETransitionsUK.commandPutIntoDrying + '?'}
            />
          </>
        ) : (
          <LockButton />
