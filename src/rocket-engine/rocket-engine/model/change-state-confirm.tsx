...
import { API_ROCKET_ENGINE_SHORT_URL } from 'infrastructure/const/urls';
import React, { ReactElement, useEffect } from 'react';
import { Text, View, TouchableHighlight, Image } from 'react-native';
import { IRocketEngine } from '../entity/rocket-engine';

export function ChangeStateRocketEngine({ route }: INavigate<TMember>): ReactElement {
  const dto = route.params.dtoTransitions || {};
  const entity = route.params.entity;
  const urlTransitions = route.params.endpoint;
  const { error, isLoading, updateResponse, resetError } = useHttp<IRocketEngine>();
  const { goBack } = useNavigate<TMember>();

  useEffect(() => {
    if (entity && urlTransitions) {
      const url = API_ROCKET_ENGINE_SHORT_URL + '/' + entity.id + urlTransitions;
      updateResponse({
        url,
        method: 'PUT',
